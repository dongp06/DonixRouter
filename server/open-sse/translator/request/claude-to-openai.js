...
function augmentReadToolSchemaIfNeeded(toolName, schema) {
  if (toolName !== "Read" || !schema || typeof schema !== "object") return schema;
  const properties = schema.properties;
  if (!properties || typeof properties !== "object") return schema;
  const pages = properties.pages;
  if (!pages || typeof pages !== "object") return schema;
  ...
  if (body.tools && Array.isArray(body.tools)) {
    result.tools = body.tools.map(tool => ({
      ...
      function: {
        ...
        parameters: augmentReadToolSchemaIfNeeded(tool.name, tool.input_schema) || { type: "object", properties: {} }
      }
    }));
  }
