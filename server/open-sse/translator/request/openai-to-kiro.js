/**
 * OpenAI to Kiro Request Translator (from n9router upstream)
 * Converts OpenAI Chat Completions format to Kiro/AWS CodeWhisperer format
 */
import { register } from "../index.js";
import { FORMATS } from "../formats.js";
import { v4 as uuidv4 } from "uuid";
import { resolveKiroModel, isThinkingEnabled, buildThinkingSystemPrefix, KIRO_AGENTIC_SYSTEM_PROMPT } from "../../config/kiroConstants.js";
...
function convertMessages(messages, tools, model) {
  ...
  const flushPending = () => {
    if (currentRole === "user") {
      ...
      const userMsg = { userInputMessage: { content: content, modelId: "" } };
      ...
      const assistantMsg = { assistantResponseMessage: { content: content } };
        ...
        if (lastMsg?.assistantResponseMessage) {
          lastMsg.assistantResponseMessage.toolUses = toolUses.map(tc => {
            if (tc.function) {
              ...
              return { toolUseId: tc.id || uuidv4(), name: tc.name, input: tc.input || {} };
            }
          });
        }
        currentRole = null;
  ...
  for (let i = 0; i < history.length; i++) {
    ...
    if (current.userInputMessage && mergedHistory.length > 0 && mergedHistory[mergedHistory.length - 1].userInputMessage) {
  ...
  if (firstHistoryTools && currentMessage?.userInputMessage && !currentMessage.userInputMessage.userInputMessageContext?.tools) {
...
export function buildKiroPayload(model, body, stream, credentials) {
  ...
  Object.defineProperty(payload, "_kiroUpstreamModel", { value: upstreamModel, enumerable: false });
