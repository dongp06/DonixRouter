...
export async function getUsageForProvider(connection, proxyOptions = null) {
  ...
  switch (provider) {
    ...
    case "kilocode":
      return await getKilocodeUsage(accessToken, providerSpecificData, proxyOptions);
    case "qwen":
...
function parseKiroQuotaData(data) {
  ...
  // Aggregate overage details from all breakdown entries
  let totalOverages = 0;
  let totalOverageCap = 0;
  let totalOverageCharges = 0;
  let overageRate = null;
  let overageCurrency = "USD";
  ...
  usageList.forEach((breakdown) => {
    ...
    // Accumulate overage stats
    totalOverages     += breakdown.currentOveragesWithPrecision || breakdown.currentOverages || 0;
    totalOverageCap   += breakdown.overageCapWithPrecision     || breakdown.overageCap     || 0;
    totalOverageCharges += breakdown.overageCharges || 0;
    if (breakdown.overageRate != null) overageRate = breakdown.overageRate;
    if (breakdown.currency)           overageCurrency = breakdown.currency;
  ...
  // Merge live overage numbers into overageConfiguration for the UI
  const baseOverageCfg = data.overageConfiguration || {};
  const overageConfiguration = {
    ...baseOverageCfg,
    // Formatted strings consumed by getKiroOverageInfo() in the frontend
    currentChargesFormatted: `$${totalOverageCharges.toFixed(2)}`,
    rateFormatted: overageRate != null ? `$${overageRate.toFixed(2)}/req` : "$0.04/req",
    overagesFormatted: totalOverageCap > 0
      ? `${Math.round(totalOverages).toLocaleString("en-US")} / ${Math.round(totalOverageCap).toLocaleString("en-US")}`
      : `${Math.round(totalOverages).toLocaleString("en-US")}`,
    // Raw numbers for consumers that want to compute themselves
    currentOverages: totalOverages,
    overageCap: totalOverageCap,
...
// Kilo Code API config
const KILOCODE_CONFIG_USAGE = {
  usageUrl: "https://api.kilo.ai/api/usage",
  profileUrl: "https://api.kilo.ai/api/profile",
};

/**
 * Kilo Code Usage - Fetch usage/quota from Kilo Code API
 */
async function getKilocodeUsage(accessToken, providerSpecificData, proxyOptions = null) {
  if (!accessToken) {
    return { message: "Kilo Code access token not available. Please re-authorize the connection." };
  }
  ...
  if (providerSpecificData?.orgId) {
    headers["X-Kilocode-OrganizationID"] = providerSpecificData.orgId;
  }

  try {
    // Try usage endpoint first
    const response = await proxyAwareFetch(KILOCODE_CONFIG_USAGE.usageUrl, {
      method: "GET",
      headers,
    }, proxyOptions);
    ...
    if (!response.ok) {
      if (response.status === 401) {
        return { message: "Kilo Code token expired. Please re-authorize." };
      }
      // Fallback: try profile endpoint for basic info
      try {
        const profileRes = await proxyAwareFetch(KILOCODE_CONFIG_USAGE.profileUrl, {
          method: "GET",
          headers,
        }, proxyOptions);
        if (profileRes.ok) {
          ...
          return {
            plan: profile.plan || profile.subscription?.plan || "Free",
            message: "Kilo Code connected. Detailed usage data not available.",
            quotas: {},
          };
        }
      } catch {}
      return { message: `Kilo Code usage API temporarily unavailable (${response.status}).` };
    }

    const data = await response.json();
    const quotas = {};

    // Parse usage data - adapt to whatever structure Kilo returns
    if (data.quotas && typeof data.quotas === "object") {
      for (const [name, quota] of Object.entries(data.quotas)) {
        quotas[name] = {
          used: quota.used || 0,
          total: quota.total || quota.limit || 0,
          remaining: (quota.total || quota.limit || 0) - (quota.used || 0),
          resetAt: parseResetTime(quota.resetAt || quota.resetsAt || quota.reset_at) || null,
          unlimited: quota.unlimited || false,
        };
      }
      ...
      for (const [name, usage] of Object.entries(data.usage)) {
        const total = usage.limit || usage.total || 0;
        const used = usage.used || usage.count || 0;
        quotas[name] = {
          used,
          total,
          remaining: Math.max(0, total - used),
          resetAt: parseResetTime(usage.resetAt || usage.resetsAt || usage.reset_at) || null,
          unlimited: usage.unlimited || false,
        };
      }
    } else if (data.limits && Array.isArray(data.limits)) {
      for (const limit of data.limits) {
        const name = limit.name || limit.type || "quota";
        const total = limit.total || limit.limit || 0;
        const used = limit.used || limit.count || 0;
        quotas[name] = {
          used,
          total,
          remaining: Math.max(0, total - used),
          resetAt: parseResetTime(limit.resetAt || limit.resetsAt || limit.reset_at) || null,
    ...
    return {
      plan: data.plan || data.subscription?.plan || data.tier || "Free",
      quotas,
    };
  } catch (error) {
    throw new Error(`Failed to fetch Kilo Code usage: ${error.message}`);
  }
}
