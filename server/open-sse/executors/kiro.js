...
const FETCH_CONNECT_TIMEOUT_MS = (() => {
  const raw = process.env.KIRO_FETCH_CONNECT_TIMEOUT_MS;
...
export class KiroExecutor extends BaseExecutor {
   ...
   */
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null, onCredentialsRefreshed = null, streamWatchdogEnabled = true }) {
      ...
      async transform(chunk, rawController) {
        ...
        while (buffer.length >= 16 && iterations < maxIterations) {
          ...
          if (eventType === "codeEvent" && event.payload?.content) {
            const chunk = {
              id: responseId, object: "chat.completion.chunk", created, model,
              choices: [{ index: 0, delta: { content: event.payload.content }, finish_reason: null }]
            };
            chunkIndex++;
            controller.enqueue(new TextEncoder().encode(`data: ${JSON.stringify(chunk)}\n\n`));
          }
            ...
            for (const singleToolUse of toolUses) {
              ...
              if (toolInput !== undefined) {
                ...
                const argsChunk = {
                  ...
                  choices: [{
                    index: 0,
                    delta: { tool_calls: [{ index: toolIndex, function: { arguments: argumentsStr } }] },
                    finish_reason: null
                  }]
                };
                chunkIndex++;
          ...
          // Emit final chunk after BOTH meteringEvent AND contextUsageEvent
          if (state.hasMeteringEvent && state.hasContextUsage && !state.finishEmitted) {
            ...
            if (!state.usage) {
              ...
              const estimatedInputTokens = state.contextUsagePercentage > 0
                ? Math.floor(state.contextUsagePercentage * 200000 / 100) : 0;
              state.usage = {
